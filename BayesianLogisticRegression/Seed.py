...
def SeedGen(d, PowerOfTwo, Stream):
    ...
    if Stream == 'iid':
        ...
        xs = np.random.uniform(0,1,(int((2**PowerOfTwo-1)/(d+1))*(d+1),d+1))

    elif Stream == 'cud':
        # Load cud point sequence
        cuds = np.loadtxt('../cuds/chen/cuds_2_{}.txt'.format(PowerOfTwo))
        
        # Create d-dimensional sequence by shifted cud sequence
        UsedLength = int(cuds.shape[0]/(d+1))*(d+1)
        ...
        xs = xs.reshape(UsedLength,d+1)
        xs = np.append(np.zeros(d+1)+1e-9,xs).reshape(UsedLength+1,d+1)
        u_rand = np.random.uniform(0,1,1)
        xs_sh = xs + u_rand
